...
(function() {
    ...
    const GitHubAPI = {
        ...
        async loadContracts() {
            const result = await this.getFile(GITHUB_CONFIG.contractsPath);
            return result ? result.content : [];
        },
        ...
        async loadUsers() {
            const result = await this.getFile(GITHUB_CONFIG.usersPath);
            return result ? result.content : [];
        },
    ...
    const DataStore = {
        ...
        async loadAll() {
            try {
                this._listings = await GitHubAPI.loadListings();
                this._contracts = await GitHubAPI.loadContracts();
                this._users = await GitHubAPI.loadUsers();
                USERS = this._users;
                return true;
            } catch (error) {
    ...
    function bindEvents() {
        ...
        $("#btnAddUser").addEventListener("click", async () => {
            ...
            await DataStore.addUser({ username, password, displayName: username, role: 'agent' });
            $("#newUsername").value = "";
            $("#newPassword").value = "";
            renderUserList();
    ...
    async function init() {
        ...
        if (!loaded) {
            toast('خطا در بارگذاری داده‌ها از گیت‌هاب. تنظیمات را بررسی کنید.', 'danger');
        }
