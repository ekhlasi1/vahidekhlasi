 ...
 * نسخه نهایی با پشتیبانی کامل از گیت‌هاب
 * ============================================================
 */
...
(function() {
    ...
    // ===== تنظیمات گیت‌هاب =====
    const GITHUB_CONFIG = {
        token: "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",
        owner: "ekhlasi1",
        repo: "vahidekhlasi",
    ...
    // ===== کاربران ثابت (برای مواقعی که گیت‌هاب در دسترس نیست) =====
    ...
    const GitHubAPI = {
        ...
        async loadUsers() {
            ...
            // اگر هیچکدام نشد، از کاربران پیش‌فرض استفاده کن
        ...
        async restoreBackup() {
            try {
                const result = await this.getFile(GITHUB_CONFIG.backupPath);
                if (!result) throw new Error('فایل پشتیبان یافت نشد.');
                return { success: true, data: result.content.data };
            } catch (error) {
    ...
    const DataStore = {
        ...
        async loadAll() {
            try {
                ...
                console.log('✅ داده‌ها با موفقیت بارگذاری شدند:', {
                    ...
                    contracts: this._contracts.length,
                ...
                // استفاده از داده‌های محلی اگر وجود داشته باشد
                const savedUsers = localStorage.getItem('ekhlasi_users_backup');
                if (savedUsers) {
                    try {
                        this._users = JSON.parse(savedUsers);
                        USERS = this._users;
                        console.log('⚠️ استفاده از داده‌های محلی برای کاربران');
                    } catch {}
                }
                return false;
            }
        ...
        async addUser(user) {
            ...
            this._users = list;
            USERS = list;
            localStorage.setItem('ekhlasi_users_backup', JSON.stringify(list));
        },
    ...
    const Auth = {
        async login(username, password) {
            try {
                ...
                // اگر پیدا نشد، از کاربران ثابت استفاده کن
    ...
    function updateGitHubStatus() {
        ...
        statusEl.innerHTML = `
            ...
            <span>✅ متصل به گیت‌هاب (${GITHUB_CONFIG.owner}/${GITHUB_CONFIG.repo})</span>
        `;
    }
    ...
    function bindEvents() {
        ...
        // تست اتصال
        const testConnection = $("#testConnectionBtn");
        if (testConnection) {
            testConnection.addEventListener("click", async () => {
                toast('در حال تست اتصال...');
                const result = await GitHubAPI.testConnection();
                if (result.success) {
                    toast('✅ اتصال به گیت‌هاب برقرار است.', 'success');
                    updateGitHubStatus();
                } else {
                    toast('❌ خطا در اتصال: ' + (result.error || 'نامشخص'), 'danger');
                }
            });
        }
